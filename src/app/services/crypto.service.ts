...
export class CryptoService {
   ...
   * Keys are stored in memory only - server stores encrypted version
   */
  async generateKeyPair(): Promise<void> {
    try {
      this.keyPair = (await window.crypto.subtle.generateKey(
        this.ALGORITHM_RSA,
        true, // extractable (needed for server upload)
        ['encrypt', 'decrypt']
      )) as CryptoKeyPair;
  ...
  async decryptPrivateKeyFromServer(
    ...
    try {
      ...
      const encryptedWithTag =
        ...
        this.base64ToArrayBuffer(encryptedData.authTag).toString();
   ...
   */
  async loadPrivateKeyFromServer(
    encryptedPrivateKeyJson: string,
    password: string
  ): Promise<void> {
    try {
      ...
      const decryptedPem = await this.decryptPrivateKeyFromServer(
      ...
      // For now, derive public key from private key if needed
      // In practice, we'll fetch recipient public keys from server during encryption
      ...
      this.keyPair = {
        privateKey,
        publicKey: tempKeyPair.publicKey, // Temporary - real public keys fetched from server
      };
